...
char buffer[255];
const char *url = "https://script.google.com/macros/s/AKfycbyHCGqYdke9koMNYHOfLdDoT-6KV2s_f8uWCOfCbHSLD1KqvjYL-_3N3kABovo7oGOa/exec";
...
bool isLTEConnected() {
    ...
    if (response.indexOf("+CREG: 0,1") != -1 || respon

    se.indexOf("+CREG: 0,2") != -1 || response.indexOf("+CREG: 0,3") != -1 ) {
...
void setup() {
    ...
    terminal.print("Response 3: ");
    terminal.print(readstr);

    AddMsg("AT+CGAUTH = 1, 1, \"pp@sim\", \"jpn\"\r\n", kASSIGN_MT, 1000, 1000);             
    while ((readSendState(0) == kSendReady) || (readSendState(0) == kSending) ||
           (readSendState(0) == kWaitforMsg))
        delay(50);
    restate = readSendState(0);
